...
{
    private List<TableOperInfo> tableOperInfoList = new ArrayList<TableOperInfo>();
    ...
    public List<TableOperInfo> parse(String sql){
        createStatement(sql);
        return tableOperInfoList;
    }
    ...
    private Node invokeParser(String name, String sql, Function<SqlBaseParser, ParserRuleContext> parseFunction)
    {
        try {
            ...
            IdentityHashMap<Expression, QualifiedName> ttt = new IdentityHashMap<>();
            TreePrinter treePrinter = new TreePrinter(ttt,System.out);
            treePrinter.print(new AstBuilder().visit(tree));
//
//            //打印所有的表及其操作
//            System.out.println("**********************************");
//            List<TableOperInfo> toiList = new ArrayList<TableOperInfo>();
            tableOperInfoList = treePrinter.getAllTableOperInfo();

//            for(int i=0;i<toiList.size();i++){
//                System.out.println("Operator: "+(toiList.get(i).getOpType()==0?"select":"not select")+"\t"+"Table: "+toiList.get(i).getTableName());
//            }
//            System.out.println("**********************************");
